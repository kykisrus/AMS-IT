...
router.post('/', auth, checkRole(['super_admin', 'it_specialist']), async (req, res) => {
  try {
    ...
    const [result] = await db.pool.query(
      `INSERT INTO equipment (inventory_number, serial_number, uuid, model, manufacturer, purchase_date, purchase_cost, depreciation_period, liquidation_value, current_status, current_owner_id, company_id, glpi_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)` ,
      [inventory_number, serial_number, uuid, model, manufacturer, purchase_date, purchase_cost, depreciation_period, liquidation_value, current_status, current_owner_id, company_id, glpi_id]
    );
    res.status(201).json({ id: result.insertId });
  } catch (error) {
    res.status(500).json({ error: 'Ошибка добавления техники' });
  }
});
