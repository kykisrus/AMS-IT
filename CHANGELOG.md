...
- Интеграция системы импорта данных
- Поддержка импорта CSV файлов
- Новые таблицы в базе данных для управления импортом: import_jobs, import_errors, import_mappings
- Индексы для оптимизации импорта данных
- Добавлен пакет csv-parse в install.sh
...
- Полностью реализована и доработана логика ролей и разрешений:
  - Чекбоксы "Является руководителем" и "Может быть МОЛ" теперь являются обычными разрешениями (is_manager, is_mol) и входят в массив permissions роли.
  - Вся логика назначения МОЛ и руководителей теперь строится на этих правах, а не на отдельном поле роли.
- Исправлена и унифицирована валидация при создании и редактировании ролей.
- Исправлена обработка ошибок и предотвращены бесконечные запросы при ошибках валидации.
- Исправлен и расширен UI:
  - Добавлен логотип на страницу входа, реализована поддержка кастомного логотипа.
  - Все выпадающие списки переведены на Material-UI (MenuItem), устранены конфликты с Ant Design.
  - Формы организаций теперь поддерживают все поля из БД: name, phone, email, website, logo_url, mol_id.
  - Исправлен выбор МОЛ и руководителя: теперь отображаются только пользователи с соответствующими правами.
- На сервере:
  - Исправлена логика получения руководителей и МОЛ (по JSON permissions).
  - В контроллере компаний добавлена проверка наличия права is_mol у выбранного пользователя.
  - Обновлён SQL-триггер check_mol_role: теперь он проверяет наличие права is_mol в permissions роли пользователя.
- Документация:
  - Обновлены и синхронизированы все разделы (README.md, STYLE_GUIDE.md, AUTH_DOCUMENTATION.md, DATABASE1.2.md).
  - Добавлено описание работы с ролями, разрешениями, триггером и формами.
- Чистка файлов:
  - Удалены устаревшие схемы и init.sql из папки database.
  - Проверена структура проекта, удалены лишние и дублирующие файлы.
- Переход с MongoDB на MySQL/MariaDB
- Адаптация структуры базы данных:
  - Замена типа JSON на LONGTEXT
  - Изменение row_number на line_number
  - Оптимизация индексов
- Обновлена конфигурация подключения к базе данных
- Обновлена документация по импорту (IMPORT.md)
- Реструктуризация документации

### Совместимость
- Все изменения полностью отражены в документации и скриптах установки.
- Проект готов к сборке и развертыванию на новой версии базы данных и Node.js.
...
- Проблема с подключением к базе данных (TypeError: db.connect is not a function)
